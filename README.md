...
- **第1〜8章**: ブラウザの中に作った擬似 Git リポジトリを、実際にコマンドを打って動かします。オフラインで遊べます。第8章は手順を教えない修了試験と自由練習です。
- **第9章**: 本物の GitHub リポジトリに対して、ブランチ作成 → コミット → Pull Request → マージまでを実行します。
...
一度開けば、あとは電波が無くても第1〜8章は遊べます（Service Worker がアプリを端末に置くため）。
...
| **⊞ 状態** | 上がコミットグラフ、下が「作業ツリー / ステージ / 直前のコミット」の3列比較。打ったコマンドに合わせてその場で動きます |
| **◎ クエスト** | いまのステージの目標・ヒント・まとめ |
| **？ 逆引き** | やりたいこと → コマンドのチートシート。タップでコピーできます |

幅が 1000px 以上の画面（PC）では **左にターミナル、右に選んだ画面** の2カラムになります。
右側で「⊞ 状態」を選ぶと、ターミナル・コミットグラフ・3つの場所が同時に見えるので、
**コマンドを打った瞬間にグラフとファイルが動くのを、画面を行き来せずに追えます**。
このときターミナルは出しっぱなしになるので、下部タブからは外れて3つになります。
...
| 第8章 修了試験 | **手順を教えない総合演習**。目指す状態だけが示され、辿り着く道は自分で決める（判定は最終状態だけ）。最後は目標なしの**自由練習** |
| 第9章 GitHub 実践 | **基礎編**: 本物のリポジトリでブランチ → コミット → PR → コメント → マージ<br>**実務編**: レビュー（自分の PR は承認できない）/ main が進んだ状態の検知 / バックマージ / マージ3方式の選択 / CI が呼ぶ API |

第1〜8章で **40 ステージ**。すべて模範解答つきの自動テストで「本当にクリアできる」ことを検証しています。

---

## 第9章（本物の GitHub を触る）について
...
```sh
npm test          # エンジン・全ステージ・現在地・救出・ロックの自動テスト（214 件）
...
js/ui/         画面（ターミナル / 状態=グラフ+ファイル / クエスト / チートシート）
js/stages/     ステージ定義（データ駆動）
js/real/       第9章: GitHub REST クライアントと安全チェック
...
**そのあとはパソコンを切ってもオフラインで遊べます**（第9章を除く）。
